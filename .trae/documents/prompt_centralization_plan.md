...
5. 修改 `mmap_optimizer/core/config.py` - 增加 `PromptsConfig` 配置段

### 阶段三：修改分析执行器

6. 修改 `mmap_optimizer/executors/analysis_executor.py` - 使用模板文件替换硬编码消息

### 阶段四：更新 CLI 和 Runner

7. 修改 `mmap_optimizer/core/cli.py` - 增加新的 prompt 参数
8. 修改 `mmap_optimizer/core/runner.py` - 传递新的 prompt 配置
