...
class ScenarioValidationError(ScenarioError):
    """Raised when a scenario directory does not satisfy the expected contract."""
...
class ScenarioConfig:
    ...
    The dataclass preserves the existing main-branch fields used by the CLI while
    adding manifest/config-inheritance metadata from the scenario PRs.
    """
...
def resolve_scenario_root(scenario: str | Path, scenarios_dir: str | Path = DEFAULT_SCENARIOS_DIR) -> Path:
    ...
    if candidate.exists() or len(candidate.parts) > 1 or candidate.is_absolute():
        return candidate
...
def read_manifest(scenario_root: str | Path) -> ScenarioManifest:
    """Read ``scenario.yaml`` if present, otherwise synthesize a compatible manifest."""
    ...
    scenario_id = str(data.get("id") or root.name)
...
def load_scenario(
    ...
    """Load a scenario and apply optional base-config inheritance.

    ``scenario`` may be either an explicit directory path or an id under
    ``scenarios_dir``. Existing callers that pass a path continue to work.
    """

    root = resolve_scenario_root(scenario, scenarios_dir)
    manifest = validate_scenario(root, scenarios_dir=scenarios_dir, require_resources=False)
    optimizer_config_name = _find_optimizer_config(root, manifest)
    config_path = _safe_relative(root, optimizer_config_name, "optimizer_config")
...
def list_scenarios(scenarios_dir: str | Path = DEFAULT_SCENARIOS_DIR) -> list[ScenarioSummary]:
    """List valid scenarios under ``scenarios_dir`` sorted by scenario id."""
...
def run_artifact_metadata(scenario: ScenarioConfig) -> dict[str, Any]:
    """Build scenario metadata suitable for embedding in run artifacts."""
...
def write_scenario_artifact_metadata(artifact_dir: str | Path, scenario: ScenarioConfig) -> Path:
    """Write scenario metadata to ``scenario_artifacts.json`` and return its path."""
...
# Backwards-compatible name used by PR #2 tests/docs.
