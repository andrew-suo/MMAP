...
class PromptOptimizationPhase:
    ...
    def _save_iteration_artifacts(
        ...
        _write_jsonl(extraction_dir / "translation_failed_attempts.jsonl", getattr(extraction_stage.patch_generation_executor, "translation_failed_attempts", []) if getattr(extraction_stage, "patch_generation_executor", None) is not None else [])
        ...
        _write_jsonl(analysis_dir / "translation_failed_attempts.jsonl", getattr(analysis_stage.patch_generation_executor, "translation_failed_attempts", []) if getattr(analysis_stage, "patch_generation_executor", None) is not None else [])
